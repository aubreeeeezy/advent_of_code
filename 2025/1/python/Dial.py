...
ROTATION_COMMAND_RE_COMPILED: Pattern[str] = compile(
    pattern=r"([LR])(\d+)"
)  # Optimizes Regex Parsing
...
class Dial:
    ...
    def _parse_rotation_command_(self, rotation_command) -> tuple[int, int]:
        ...
        match: Optional[Match[str]] = ROTATION_COMMAND_RE_COMPILED.match(
            string=rotation_command
        )
        if match:
    ...
    def rotate(self, rotation_command: str) -> int:
        ...
        """
        relative_steps, times_passed_zero = self._parse_rotation_command_(
        ...
        if self.position == 0 and relative_steps:  # Position is zero and has moved.
            ...
            if (
                initial_position == 0 and raw_position < 0
            ):  # if we start at 0 and go negative we have not passed zero
                pass
            else:
                times_passed_zero += 1
